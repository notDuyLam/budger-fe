...
- `is_credit_card` (Boolean, mặc định `false`) - Cho phép số dư âm; không khóa số dư khi chi tiêu.
- `is_hidden` (Boolean, mặc định `false`) - Ví ẩn không hiển thị trong Net Worth và không xuất hiện khi tạo giao dịch.
- `is_balance_masked` (Boolean, mặc định `false`) - Khi `true`, số dư chi tiết ví ẩn được che bằng `****`.
- `color` (String, nullable) - Class gradient màu ví.
- `icon` (String, nullable) - Tên icon ví.
...
- `type` (Enum: `INCOME`, `EXPENSE` - Lưu trữ mặc định trong DB, nhưng ở giao diện xử lý trung lập cho phép dùng chung cho mọi loại giao dịch)
- `created_at`
...
Thiết kế 5 màn hình chính (không có màn Account riêng biệt), sử dụng Bottom Navigation Bar trên Mobile và Sidebar trên PC. Thông tin tài khoản người dùng hiển thị trong Dropdown/Popover trên Header.

### **Màn 1: Dashboard (Tổng quan)**

- **Header:** Tổng tài sản (Sum balance của tất cả ví - không tính ví `is_hidden = true`).
- **Section 1:** Danh sách Ví (Horizontal scroll trên Mobile, Grid 2 cột trên PC). Hiển thị tên ví và số dư. Ví `is_hidden` không hiển thị số dư và không được tính vào Net Worth. Click vào ví → redirect sang Transactions History với filter đã áp sẵn wallet đó.
...
### **Màn 4: Category Management (Quản lý Danh mục)**

- **Cấu trúc:** Giao diện độc lập.
- **Section 1 - Form:** Cho phép Thêm/Sửa danh mục với tên, loại (INCOME/EXPENSE), và icon tùy chỉnh. Hỗ trợ AI gợi ý Icon dựa trên tên danh mục.
- **Section 2 - List:** Liệt kê các danh mục hiện tại kèm theo icon tương ứng, cho phép cập nhật hoặc xóa danh mục (sử dụng custom toast xác nhận).

### **Màn 5: Analytics (Báo cáo)**

- **Controls:** Chọn khoảng thời gian (Tháng này, Tháng trước, Tuần này).
- **Metrics:** Tổng Thu, Tổng Chi.
- **Charts:**
    - **Pie Chart (SVG Donut):** Cơ cấu theo Category. Có nút toggle chuyển đổi giữa **Chi tiêu (Expense)** và **Thu nhập (Income)**. Click vào phần trạng thái để lọc chi tiết.
    - **Bar Chart:** So sánh Thu / Chi theo từng ngày trong tuần hoặc từng tuần trong tháng.
...
- **Lưu trữ:**
  ...
  - Lỗi số dư âm và lỗi trùng đối tác nợ phải hiển thị **Toast Notification** trực quan thay vì dùng `console.error` hoặc `alert`.
...
- **Tạo mới:** Tại màn hình Lịch sử giao dịch (History Tab), nút "+ Add Transaction" mở Modal Form chứa các trường:
  ...
  - `Amount` (Số tiền - Bắt buộc, sử dụng **Money Input Formatter** tự động định dạng dấu chấm phân cách nghìn)
  - `Type` (Loại: Thu, Chi, Chuyển khoản, Cho vay, Đi vay, Trả nợ - Bắt buộc)
  - `Wallet Account` (Tài khoản ví thanh toán - Bắt buộc; chỉ hiển thị ví không `is_hidden`)
  - `Category` (Danh mục phân loại - Bắt buộc với INCOME/EXPENSE; hiển thị danh sách trung lập dùng chung)
  - `Due Date` (Ngày hẹn trả - chỉ hiện với giao dịch nợ DEBT_LENT/DEBT_BORROWED - Tùy chọn)
...
#### **5.1.4. Nhận Diện TRANSFER qua AI**
- AI nhận diện và phân loại giao dịch `TRANSFER` (Chuyển khoản giữa các ví).
- Ví dụ: *"Chuyển 500k từ ví momo qua thẻ hsbc credit"* → `type: TRANSFER`, `wallet` (nguồn: momo), `to_wallet` (đích: hsbc credit).
- Hệ thống cập nhật số dư cả 2 ví sau khi lưu.

#### **5.1.5. Nhóm Lịch sử Chat AI (Chat History Grouping)**
- Màn hình lịch sử chat AI nhóm theo: **Today**, **This Week**, **This Month**.
- Dữ liệu chat cũ hơn chỉ tải khi user chủ động yêu cầu.
